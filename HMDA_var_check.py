...
# Load data
## Prelims
path = r'X:/My Documents/Data/Data_HMDA/LAR/'

## Define column dtypes for specific columns
dtypes_col = {'state_code':'str', 'county_code':'str'}

## Load the data 
df_chunk = pd.read_csv(path + 'hmda_2010_nationwide_originated-records_codes.zip', index_col = 0, chunksize = 1e6, \
...
for chunk in df_chunk:
    ...
    chunk_filtered = chunk[(~chunk.agency_code.isin([5,7,9]))].\
...
#------------------------------------------------------------
# Prelims
fips_count = pd.DataFrame(df.groupby('fips').fips.count())
...
pio.write_image(fig, 'Figures/Number_originations_map_2010.png') 
...
## Load the data 
df_chunk = pd.read_csv(path + 'hmda_2017_nationwide_originated-records_codes.zip', index_col = 0, chunksize = 1e6, \
                 na_values = ['NA   ', 'NA ', '...',' ','NA  ','NA      ','NA     ','NA    '], dtype = dtypes_col)
...
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0}, width = 1500, height = 900,
                   ...
                   ticktext=['0','{}k'.format(np.floor(fips_count.iloc[:,1].max() / 1e3).astype(str)[:-2])]),
                   title=go.layout.Title(
                   text='Number of Mortgage Originations per County (2017)',
                   xref='paper',
                   x=0.1,
                   y=0.95)
                  )
pio.write_image(fig, 'Figures/Number_originations_map_2017.png') 