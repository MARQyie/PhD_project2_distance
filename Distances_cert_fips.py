ch# Calculate the minimum distance between all CERT and fips
...
def readHMDA(year):
    ...
    filename = 'Data/data_hmda_{}.csv.gz'.format(year)
...
# Minimum distance       
def minDistanceLenderBorrower(borrow_fips, lender_fips, distance):
...
# Distance df
df_distances = pd.read_csv('Data/data_all_distances.csv.gz',\
...
if __name__ == "__main__":
    distance_list = Parallel(n_jobs=num_cores)(delayed(test)(cert, fips) for cert, fips in inputs)     
...
# Save df
df.to_csv('Data/data_min_distances.csv.gz', index = False, compression = 'gzip')
