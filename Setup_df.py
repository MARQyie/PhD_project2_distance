...
vecMinDistanceLenderBorrower = np.vectorize(minDistanceLenderBorrower, excluded = range(2,6))

@njit(parallel=True)
def minDistanceLenderBorrowerNumba(hmda_cert,hmda_fips,sod_fips,sod_cert,dist_fips1,dist_fips2,dist_dist):
    ''' This methods calculates the minimum distance between a lender (a specific
        branche of a bank or thrift) and a borrower (a unknown individual) based on
        the respective fips codes. Calls upon the distance matrix calculated by the
        haversine formula.
    ...
    for i in prange(n):
        ...
        ## Make a subset of branches where the FDIC certificates in both datasets match  
        branches = sod_fips[sod_cert == hmda_cert[i]]
            
        ## Get the minimum distance
        output[i] = np.min(dist_dist[(dist_fips1 == hmda_fips) & (isin_nb(np.array(dist_fips2),branches))])
    
    return(output)

for val in unique_coorcomb.fips_2.unique()[:10]: 
    if '01001' == val: test = val; break
...
for year in range(start,end + 1):
    ...
    df_main['distance'] = minDistanceLenderBorrowerNumba(df_main.cert.to_numpy(dtype = int),\
