...
'''
    ...
    1. Reads in the SOD dataframes for each year-end from 2004 to 2017
...
file_pop_2000 = r'co-est00int-tot.csv'
file_pop_2010 = r'co-est2019-alldata.csv'
usecols_2000 = ['STATE', 'COUNTY'] + ['POPESTIMATE{}'.format(year) for year in range(start, 2010)]
usecols_2010 = ['STATE', 'COUNTY'] + ['POPESTIMATE{}'.format(year) for year in range(2010, end + 1)]

df_pop_2000 = pd.read_csv(path_pop + file_pop_2000, encoding = "ISO-8859-1", usecols = usecols_2000)
df_pop_2010 = pd.read_csv(path_pop + file_pop_2010, encoding = "ISO-8859-1", usecols = usecols_2010)

# Merge the dfs
df_pop = df_pop_2000.merge(df_pop_2010, how = 'left', on = ['STATE','COUNTY'])
...
df_pop_res['date'] = df_pop_res.date.str[-4:].astype(int)

#------------------------------------------------------------
# df_area
path_area = r'D:/RUG/Data/US_Census_Bureau/State-based_files/'
file_area_2000 = r'State-based_files_census2000.csv'
file_area_2010 = r'State-based_files_census2010.csv'

df_area_2000 = pd.read_csv(path_area + file_area_2000, dtype = {'GEOID':'str'}, index_col = 0)
df_area_2010 = pd.read_csv(path_area + file_area_2010, dtype = {'GEOID':'str'}, index_col = 0)

# Correct GEOID
df_area_2000.GEOID = df_area_2000.GEOID.str.zfill(5)
df_area_2010.GEOID = df_area_2010.GEOID.str.zfill(5)
...
# Make area var
df_area_2000['area'] = df_area_2000.AREALAND.divide(1e6)
df_area_2010['area'] = df_area_2010.AREALAND.divide(1e6)

#------------------------------------------------------------
# df_pop

# Correct population with area
df_pop_area_2000 = df_pop_res.loc[df_pop_res.date < 2010].merge(df_area_2000[['GEOID', 'area']], how = 'inner', left_on = 'fips', right_on = 'GEOID')
df_pop_area_2010 = df_pop_res.loc[df_pop_res.date >= 2010].merge(df_area_2010[['GEOID', 'area']], how = 'inner', left_on = 'fips', right_on = 'GEOID')
df_pop_area = pd.concat([df_pop_area_2000,df_pop_area_2010], ignore_index=True)
