...
df_msa = pd.read_csv('Data/data_msa_popcenter.csv', index_col = 0, \
                     dtype = {'fips':'str','cbsa_code':'str'}) 
df_cdd = pd.read_csv('Data/sf12010countydistancemiles.csv',\
...
df_cdd = df_cdd[(df_cdd.county1.isin(msa_fips)) & (df_cdd.county2.isin(msa_fips))]

# Transform mi to km
mi_to_km = 1 / 0.62137
df_cdd.mi_to_county = df_cdd.mi_to_county * mi_to_km

# Rename columns
df_cdd = df_cdd.rename(columns = {'county1':'fips_1', 'mi_to_county':'distance_cdd', 'county2':'fips_2'})
...
# Add the coordinates
## Fips 1
df_cdd = df_cdd.merge(unique_fips, how = 'left', left_on = 'fips_1', right_on = 'fips').rename(columns = {'latitude':'lat1','longitude':'long1'})
df_cdd.drop(columns = ['fips'], inplace = True)

## Fips 2
df_cdd = df_cdd.merge(unique_fips, how = 'left', left_on = 'fips_2', right_on = 'fips').rename(columns = {'latitude':'lat2','longitude':'long2'})
df_cdd.drop(columns = ['fips'], inplace = True)
...
df_cdd.to_csv('Data/data_all_distances_v2.csv.gz', index = False, compression = 'gzip')
